import { Result, Success, Failure, isSuccess, isFailure, dangerouslyUnwrap, unwrapOr, map } from './result';
...
describe('Result type utilities', () => {
  ...
  describe('dangerouslyUnwrap', () => {
    ...
    it('should throw for error result', () => {
      expect(() => dangerouslyUnwrap(Failure('error'))).toThrow('Attempted to unwrap an error result');
    });
  });
  ...
  describe('map', () => {
    ...
    it('should handle type transformations', () => {
      const result = Success('5');
      const mapped = map(result, (x: string) => parseInt(x));
